...
alter table public.kiosk_devices
add constraint kiosk_devices_desired_mode_check check (
  desired_mode in (
    'active',
    'paused',
    'maintenance',
...
alter table public.kiosk_devices
add constraint kiosk_devices_applied_mode_check check (
  applied_mode in (
    'active',
    'paused',
    'maintenance',
...
alter table public.kiosk_devices
add constraint kiosk_devices_config_version_order_check check (
  desired_config_version >= 1
...
alter table public.kiosk_devices
add constraint kiosk_devices_configuration_error_length_check check (
  configuration_error is null
  or char_length(configuration_error) between 1 and 500
);
...
-- organization_id.
alter table public.kiosk_devices
add constraint kiosk_devices_desired_survey_fk foreign key (desired_survey_id, organization_id) references public.surveys (id, organization_id) on delete set null;
alter table public.kiosk_devices
add constraint kiosk_devices_applied_survey_fk foreign key (applied_survey_id, organization_id) references public.surveys (id, organization_id) on delete set null;
-- =====================================================
-- 4. INDEXES
-- =====================================================
-- Supports the two composite FKs above on the referencing side, matching the
-- existing kiosk_devices_survey_org_idx.
create index if not exists kiosk_devices_desired_survey_org_idx on public.kiosk_devices (desired_survey_id, organization_id);
create index if not exists kiosk_devices_applied_survey_org_idx on public.kiosk_devices (applied_survey_id, organization_id);
...
create index if not exists kiosk_devices_pending_config_idx on public.kiosk_devices (organization_id)
...
create or replace function public.kiosk_resolve_device_credential(p_raw_credential text) returns uuid language plpgsql stable security definer
set search_path = public,
  pg_temp as $$
declare v_device_id uuid;
v_ok boolean;
begin if p_raw_credential is null
or length(p_raw_credential) < 8 then raise exception 'Invalid device credential' using errcode = 'insufficient_privilege';
end if;
select v.kiosk_device_id into v_device_id
from public.validate_kiosk_device_credential(p_raw_credential) v
limit 1;
if v_device_id is null then raise exception 'Invalid device credential' using errcode = 'insufficient_privilege';
end if;
select (
    kd.credential_revoked_at is null
    and kd.status not in ('revoked', 'archived')
  ) into v_ok
from public.kiosk_devices kd
where kd.id = v_device_id;
if v_ok is not true then raise exception 'Device credential revoked' using errcode = 'insufficient_privilege';
end if;
return v_device_id;
end $$;
comment on function public.kiosk_resolve_device_credential(text) is 'Resolves a raw kiosk device credential to a device id, rejecting unknown and revoked credentials. Internal: not granted to client roles, and never returns credential material.';
-- =====================================================
...
create or replace function public.kiosk_sanitize_configuration_error(p_error text) returns text language sql immutable as $$
select nullif(
    left(
      btrim(
        regexp_replace(coalesce(p_error, ''), '[[:cntrl:]]+', ' ', 'g')
      ),
      500
    ),
    ''
...
$$;
comment on function public.kiosk_sanitize_configuration_error(text) is 'Bounds and strips control characters from device-reported configuration error text.';
-- =====================================================
...
create or replace function public.get_kiosk_desired_configuration(p_raw_credential text) returns table (
...
set search_path = public,
  pg_temp as $$
declare v_device_id uuid;
begin v_device_id := public.kiosk_resolve_device_credential(p_raw_credential);
return query
select kd.id,
  kd.organization_id,
...
comment on function public.get_kiosk_desired_configuration(text) is 'Returns the calling kiosk device its own desired and applied configuration. Device identity comes solely from the credential, so no device can read another device or another organization.';
-- =====================================================
...
begin if p_config_version is null
or p_config_version < 1 then raise exception 'Configuration version must be a positive integer' using errcode = 'invalid_parameter_value';
end if;
...
select kd.desired_config_version,
  kd.applied_config_version into v_desired,
  v_applied
from public.kiosk_devices kd
where kd.id = v_device_id for update;
if p_config_version > v_desired then raise exception 'Cannot acknowledge a configuration version that was never issued' using errcode = 'invalid_parameter_value';
end if;
if p_config_version = v_desired
and p_config_version > v_applied then
...
comment on function public.acknowledge_kiosk_configuration(text, bigint) is 'Records that the calling kiosk applied a configuration version. Future versions are rejected; repeated or stale acknowledgements are idempotent no-ops.';
-- =====================================================
...
v_device_id := public.kiosk_resolve_device_credential(p_raw_credential);
select kd.desired_config_version into v_desired
from public.kiosk_devices kd
where kd.id = v_device_id for update;
...
if p_config_version > v_desired then raise exception 'Cannot report failure for a configuration version that was never issued' using errcode = 'invalid_parameter_value';
end if;
v_error := coalesce(
...
comment on function public.report_kiosk_configuration_failure(text, bigint, text) is 'Records a sanitized, bounded configuration failure for the calling kiosk without changing desired or applied state, so the device can retry.';
-- =====================================================
-- 10. RPC: HEARTBEAT
-- =====================================================
...
create or replace function public.record_kiosk_heartbeat(p_raw_credential text, p_applied_mode text default null) returns table (
  kiosk_device_id uuid,
...
begin if p_applied_mode is not null
and p_applied_mode not in (
  ...
  're_enrollment_required',
  'revoked'
) then raise exception 'Unsupported kiosk operating mode' using errcode = 'invalid_parameter_value';
end if;
v_device_id := public.kiosk_resolve_device_credential(p_raw_credential);
update public.kiosk_devices kd
...
select kd.id,
  ...
  kd.applied_mode,
  (
    kd.applied_config_version < kd.desired_config_version
  ) as configuration_pending
from public.kiosk_devices kd
where kd.id = v_device_id;
end $$;
comment on function public.record_kiosk_heartbeat(text, text) is 'Records liveness for the calling kiosk and optionally its currently applied mode. Cannot alter desired configuration or any version.';
-- =====================================================
...
create or replace function public.get_kiosk_configuration_state(p_organization_id uuid) returns table (
  kiosk_device_id uuid,
  device_name text,
  status text,
  desired_config_version bigint,
...
set search_path = public,
  pg_temp as $$
begin if p_organization_id is null then raise exception 'Organization is required' using errcode = 'invalid_parameter_value';
end if;
if not public.kiosk_admin_can_manage_org(p_organization_id) then raise exception 'Not authorized to view kiosk configuration for this organization' using errcode = 'insufficient_privilege';
end if;
return query
select kd.id,
  kd.device_name,
...
comment on function public.get_kiosk_configuration_state(uuid) is 'Returns kiosk configuration and liveness state for one organization to its authorized administrators. Never returns credential material.';
-- =====================================================
...
from public,
  anon,
  authenticated;
revoke all on function public.report_kiosk_configuration_failure(
  text,
  bigint,
  text
)
...
grant execute on function public.report_kiosk_configuration_failure(
  text,
  bigint,
  text
) to service_role;
