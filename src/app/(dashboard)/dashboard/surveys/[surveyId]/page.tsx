...
export default async function SurveyDetailPage({ params, searchParams }: { params: Promise<{ surveyId: string }>; searchParams: Promise<{ error?: string; published?: string; archived?: string; saved?: string }> }) {
  ...
  const kioskUrl = `${env.NEXT_PUBLIC_APP_URL}/kiosk/${survey.publicSlug}`;
  const publicUrl = `${env.NEXT_PUBLIC_APP_URL}/feedback/${survey.publicSlug}`;
  return <div className="grid gap-6"><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-brand">{survey.status === "active" ? "Published" : survey.status}</p><h1 className="mt-2 text-2xl font-bold tracking-tight text-foreground">{survey.draft.titleEn}</h1><p className="mt-1 text-muted" dir="rtl">{survey.draft.titleAr}</p></div><div className="flex flex-wrap gap-2"><Link className="rounded-lg border border-border bg-white px-4 py-2 font-medium" href={`/dashboard/surveys/${surveyId}/analytics`}>Analytics</Link><Link className="rounded-lg border border-border bg-white px-4 py-2 font-medium" href={`/dashboard/surveys/${surveyId}/distribution`}>Distribution</Link>{canManage && survey.status === "draft" ? <Link className="rounded-lg border border-border bg-white px-4 py-2 font-medium" href={`/dashboard/surveys/${surveyId}/edit`}>Edit</Link> : null}</div></header>
    ...
    {survey.status === "active" ? (
      <section className="rounded-xl border border-border bg-white p-6">
        <h2 className="text-base font-semibold text-foreground">Collection Links</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div className="rounded-lg border border-border bg-background p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted">iPad Kiosk Link</p>
            <p className="mt-1 text-sm font-medium text-foreground break-all">{kioskUrl}</p>
            <div className="mt-2">
              <CopyLinkButton value={kioskUrl} labelEn="Copy kiosk link" labelAr="نسخ رابط الكشك" copiedLabelEn="Copied!" copiedLabelAr="تم النسخ!" />
            </div>
            <p className="mt-2 text-xs text-muted">Open this URL on the iPad to start the kiosk survey experience with welcome screen.</p>
          </div>
          <div className="rounded-lg border border-border bg-background p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted">Public Feedback Link</p>
            <p className="mt-1 text-sm font-medium text-foreground break-all">{publicUrl}</p>
            <div className="mt-2">
              <CopyLinkButton value={publicUrl} labelEn="Copy public link" labelAr="نسخ الرابط العام" copiedLabelEn="Copied!" copiedLabelAr="تم النسخ!" />
            </div>
            <p className="mt-2 text-xs text-muted">Share this link via email, WhatsApp, or SMS. Customers open it on any device.</p>
          </div>
        </div>
      </section>
    ) : null}
