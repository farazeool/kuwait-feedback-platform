...
export default async function SurveyDetailPage({ params, searchParams }: { params: Promise<{ surveyId: string }>; searchParams: Promise<{ error?: string; published?: string; archived?: string; saved?: string }> }) {
  ...
  return <div className="grid gap-7"><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold text-brand">{survey.status === "active" ? "Published" : survey.status}</p><h1 className="mt-2 text-3xl font-bold">{survey.draft.titleEn}</h1><p className="mt-1 text-muted" dir="rtl">{survey.draft.titleAr}</p></div><div className="flex flex-wrap gap-2"><Link className="rounded-xl border border-border bg-white px-4 py-2 font-semibold" href={`/dashboard/surveys/${surveyId}/analytics`}>Analytics</Link><Link className="rounded-xl border border-border bg-white px-4 py-2 font-semibold" href={`/dashboard/surveys/${surveyId}/distribution`}>Distribution</Link>{canManage && survey.status === "draft" ? <Link className="rounded-xl border border-border bg-white px-4 py-2 font-semibold" href={`/dashboard/surveys/${surveyId}/edit`}>Edit</Link> : null}</div></header>
    ...
    {canManage ? <div className="flex flex-wrap gap-3">{survey.status === "draft" || survey.status === "archived" ? <form action={publishSurvey}><input type="hidden" name="surveyId" value={surveyId} /><button className="rounded-xl bg-brand px-4 py-3 font-semibold text-white">{survey.status === "archived" ? "Restore and publish" : "Publish"}</button></form> : null}{survey.status === "active" ? <form action={archiveSurvey}><input type="hidden" name="surveyId" value={surveyId} /><button className="rounded-xl border border-border bg-white px-4 py-3 font-semibold">Archive</button></form> : null}<form action={duplicateSurvey}><input type="hidden" name="surveyId" value={surveyId} /><button className="rounded-xl border border-border bg-white px-4 py-3 font-semibold">Duplicate to draft</button></form></div> : <p className="rounded-xl bg-background p-4 text-sm text-muted">Your role has read-only survey access.</p>}
  </div>;
}
