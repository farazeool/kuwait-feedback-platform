...
export default async function SurveysPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; location?: string }> }) {
  ...
  const { context, rows } = await listSurveyGroups({ ...filters, search: filters.q });
  ...
  return (
    <div className="grid gap-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        {canManageSurveyStructure(role) ? <Link prefetch scroll={false} className="rounded-lg bg-brand px-4 py-3 font-semibold text-white" href="/dashboard/surveys/new">New survey</Link> : null}
      </header>
