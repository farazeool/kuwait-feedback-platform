...
export default async function LocationDetailPage({ params, searchParams }: { params: Promise<{ locationId: string }>; searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className="grid gap-7"><header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-brand">Location analytics</p><h1 className="mt-2 text-3xl font-bold">{location.name_en}</h1><p dir="rtl" className="text-muted">{location.name_ar}</p><p className="mt-2 text-sm text-muted">{scoped.range.from} – {scoped.range.to} · Asia/Kuwait</p></div>{canManage ? <Link href={`/dashboard/locations/${locationId}/edit`} className="rounded-xl border border-border px-4 py-2 font-bold">Edit location</Link> : null}</header><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Responses" value={String(scoped.overview.selected_responses)} /><MetricCard label="Normalized rating" value={scoped.overview.average_normalized === null ? "No data" : `${scoped.overview.average_normalized}%`} /><MetricCard label="Open alerts" value={String(scoped.overview.open_alert_count)} /><MetricCard label="Organization comparison" value={orgAverage === null || orgAverage === undefined || scoped.overview.average_normalized === null ? "Insufficient data" : `${(scoped.overview.average_normalized - orgAverage).toFixed(1)} pts`} detail={organization ? "Difference from permitted organization average" : "Restricted for location managers"} /></div>
