...
export default async function LocationsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className="grid gap-7"><header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-brand">Branch intelligence</p><h1 className="mt-2 text-3xl font-bold">Locations</h1><p className="mt-2 text-muted">Exact response counts accompany every normalized average.</p></div>{canManage ? <Link href="/dashboard/locations/new" className="rounded-xl bg-brand px-4 py-3 font-bold text-white">New location</Link> : null}</header><AnalyticsFilters values={{ ...raw, ...result.filters }} organizations={result.organizations} locations={result.locations} surveys={result.surveys} action="/dashboard/locations" />
