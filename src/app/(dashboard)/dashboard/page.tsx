...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  if (!analytics) return <div className="rounded-3xl border border-dashed border-border p-10 text-center text-muted">Create an organization before viewing analytics.</div>;
  ...
  return (
    <div className="grid gap-7">
      ...
      <section className="overflow-x-auto rounded-3xl border border-border bg-white"><div className="p-5"><h2 className="text-xl font-bold">Recent responses</h2></div><table className="w-full min-w-[650px] text-sm"><thead className="bg-background text-muted"><tr><th className="p-3 text-start">Submitted</th><th className="p-3 text-start">Survey</th><th className="p-3 text-start">Location</th><th className="p-3 text-start">Rating</th><th className="p-3 text-start">Workflow</th></tr></thead><tbody>{analytics.recent_responses.map((response) => <tr key={response.id} className="border-t border-border"><td className="p-3"><Link className="font-semibold text-brand" href={`/dashboard/responses/${response.id}`}>{formatKuwaitDateTime(response.submitted_at)}</Link></td><td className="p-3">{response.survey_title}</td><td className="p-3">{response.location_name}</td><td className="p-3">{response.normalized_rating === null ? "—" : `${response.normalized_rating}%`}</td><td className="p-3">{response.workflow_status.replaceAll("_", " ")}</td></tr>)}</tbody></table>{analytics.recent_responses.length === 0 ? <p className="p-6 text-sm text-muted">No responses match the selected filters.</p> : null}</section>
    </div>
  );
}

function ChartCard({ title, summary, children }: { title: string; summary: string; children: React.ReactNode }) {
  return <section className="grid gap-5 rounded-3xl border border-border bg-white p-6"><div><h2 className="text-xl font-bold">{title}</h2><p className="mt-1 text-sm text-muted">{summary}</p></div>{children}</section>;
}
