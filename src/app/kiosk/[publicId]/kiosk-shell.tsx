...
import type { PublicSurveyFormProps } from "@/components/feedback/public-survey-form";
...
export function KioskShell({
  ...
  const [idleTimerRef, setIdleTimerRef] = useState<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    const requestFullscreen = async () => {
      try {
        ...
        // Fullscreen not allowed (e.g., not user-initiated), continue anyway
      }
    };
    requestFullscreen();
  ...
  // Idle timeout detection - reset on any user interaction
  const resetIdleTimer = useCallback(() => {
    if (idleTimerRef) clearTimeout(idleTimerRef);
    const timer = setTimeout(() => {
      // Reset to start: clear form state and show form again
      setShowForm(true);
      setCountdown(THANK_YOU_COUNTDOWN_SECONDS);
    }, IDLE_TIMEOUT_MS);
    setIdleTimerRef(timer);
  }, [idleTimerRef]);

  useEffect(() => {
    // Attach listeners to document for all interactions
  ...
  }, [resetIdleTimer, idleTimerRef]);

  // Handle form submission complete - show thank you screen with countdown
      ...
      countdownRef.current = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            ...
            // After countdown, reset form and show it again
            setShowForm(true);
            setCountdown(THANK_YOU_COUNTDOWN_SECONDS);
  ...
  // Prevent navigation/back button in kiosk mode
  useEffect(() => {
  ...
  if (showForm) {
    return (
      <main
        ...
        role="application"
        aria-label={isArabic ? "استطلاع كشك" : "Kiosk Survey"}
      >
        {/* Fullscreen hint - only show briefly on non-fullscreen */}
        {!isFullscreen && (
          <div
          ...
          >
            <div className="mx-auto max-w-2xl px-6 text-center">
              <p className="text-lg font-medium text-foreground">
                {isArabic ? "اضغط للشاشة الكاملة" : "Tap for fullscreen"}
              </p>
              <p className="mt-2 text-sm text-muted">
                {isArabic ? "يبدأ الاستطلاع تلقائياً" : "Survey starts automatically"}
              </p>
            </div>
          </div>
        )}
        ...
        <PublicSurveyForm
          ...
          channel="kiosk"
          autoReset={false}
          onKioskComplete={handleSubmitComplete}
        />
      </main>
    );
  }
  ...
  return (
    <main
      dir={isArabic ? "rtl" : "ltr"}
      lang={locale}
      className="fixed inset-0 bg-white grid place-items-center"
      role="status"
      aria-live="polite"
      aria-label={isArabic ? "شكراً لك" : "Thank you"}
    >
      <div className="mx-auto max-w-2xl px-6 text-center">
        <div
          style={{
          ...
          className="mx-auto grid size-24 place-items-center rounded-full text-2xl"
          aria-hidden="true"
        >
          {CHECKMARK_SVG}
        </div>
        ...
        <div
          className="mt-4 text-5xl font-mono font-bold tabular-nums"
        ...
        <p className="mt-4 text-sm text-muted" aria-hidden="true">
          {isArabic
            ? `إعادة التحميل خلال ${countdown} ثوانٍ`
