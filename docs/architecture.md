...
GitHub CI runs dependency installation, ESLint, type checking, Vitest, localization parity, secret and browser/server-boundary scans, client-bundle scanning, migration ordering validation, production build, high-severity dependency audit, and a separate official Docker-backed Supabase reset/lint/pgTAP/RLS/type-generation job. `/api/health/live` and `/api/health/ready` are deliberately minimal, no-store JSON endpoints; readiness reports only a generic unavailable state when required runtime configuration cannot be parsed. Production headers include CSP, anti-framing, `nosniff`, strict referrer/permissions policy, cross-origin isolation controls compatible with the app, and HSTS only in production.
...
Analytics are calculated in PostgreSQL through bounded, typed RPCs and are never assembled from unrestricted browser queries. `assert_analytics_scope` validates the organization, optional location and survey, caller scope, date ordering, and a maximum 366-day interval. Organization analysts can read organization summaries; location-scoped analysts and location managers can read only assigned branches. Anonymous callers and unrelated tenants receive no aggregate values, including zero-count side channels.

Rating analytics normalize each survey response onto a documented 0–100 scale using that survey's rating question bounds: `(value - min) / (max - min) * 100`. The dashboard lists the source scales so users are not misled when comparing 1–5 and 0–10 questions. Raw question pages retain their own scale. NPS is not calculated; it may be introduced only for a deliberately configured 0–10 recommendation question with documented promoter/detractor semantics. All storage is UTC, while filter boundaries, buckets, and display use `Asia/Kuwait` calendar time.

Branch ranking requires at least five responses in both the current comparison and, for improvement/decline, the preceding equal-length period. Counts are displayed beside averages. Below that threshold the UI says “insufficient data”; it does not imply statistical significance. Live indexed aggregation is appropriate initially, with rollups considered only after measured plans and production-like volumes justify them.

Alert and response changes cross server actions into `SECURITY DEFINER` workflow functions. Owners/admins may operate across their organization, location managers only within assigned branches, and analysts remain read-only. Alert states are `open`, `acknowledged`, `resolved`, and `dismissed`. Response states are `unread`, `reviewed`, `action_required`, and `resolved`; tags, assignment, timestamps, and notes are internal. Direct ordinary table updates are revoked. Focused triggers write redacted audit metadata and never copy customer answers or note contents into logs.

CSV endpoints reapply the caller's RLS scope and selected filters, stream in pages, cap output at 10,000 rows and 366 days, and audit the export kind plus non-sensitive scope. Output includes a UTF-8 BOM for Arabic-capable Excel imports, UTC and Kuwait-local timestamps, neutralizes spreadsheet formulas, and uses safe filenames. Credentials, auth metadata, rate-limit fingerprints, IP addresses, tokens, and internal database identifiers are excluded. Responses and answers remain sensitive even when authorized; production retention and export-role policy need formal review.

Scale expectations are deliberately simple: at 1,000 responses, indexed live aggregates should be immediate; at 10,000, bounded filters and pagination remain mandatory and query plans are monitored; at 100,000, asynchronous exports and measured daily rollups should be evaluated without weakening tenant keys. Cache entries, if later introduced, must include user authorization scope, organization/location access, and the full filter set.
